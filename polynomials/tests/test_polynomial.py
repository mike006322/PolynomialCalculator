...
from polynomials.polynomial import (
    NonFactor,
    Polynomial,
    divides,
    division_algorithm,
...
class TestPolynomial(unittest.TestCase):
    ...
    def test_terms(self):
        s = Polynomial("5x^2y + xy^2 + y^2")
        self.assertEqual(
            list(s.terms()), [Polynomial("5x^2y"), Polynomial("xy^2"), Polynomial("y^2")]
        )

    def test__iter__(self):
        S = Polynomial("x^2y + xy^2 + y^2")
        # All terms should have header and float coefficients
        expected = [Polynomial("x^2y"), Polynomial("xy^2"), Polynomial("y^2")]
    ...
    def test_division_string(self):
        ...
        e = Polynomial("y^2 - 1")
        res1 = (
            "x^2y + xy^2 + y^2 = (x + y)*(xy - 1.0) + (1.0)*(y^2 - 1.0) + (remainder:) x + y + 1.0"
        )
    ...
    def test_gcd(self):
        ...
        self.assertEqual(
            gcd(Polynomial("x", 2) ** 2 - Polynomial("x", 2), Polynomial("x^3 + x + 1", 2)),
            Polynomial("1"),
        )
        self.assertEqual(
            gcd(Polynomial("x^4 + x^2 + 1.0", 2), Polynomial("x^4-x", 2)), Polynomial("x^2 + x + 1")
        )
        self.assertEqual(
            gcd(
                Polynomial("x", 2) ** (2**3) - Polynomial("x", 2), Polynomial("x^5 + x^3 + 1.0", 2)
            ),
            Polynomial("1"),
        )
        self.assertEqual(
            gcd(Polynomial("x^2-2x+1"), Polynomial("x^2-1")) % Polynomial("x-1").degree(), 0
        )
    ...
    def test_hessian(self):
        ...
        self.assertEqual(
            [[float(x) for x in row] for row in f.hessian(x=1, y=2)], [[4.0, 14.0], [14.0, 24.0]]
        )
