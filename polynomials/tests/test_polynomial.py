import unittest
from polynomials.polynomial import Polynomial, NonFactor, division_algorithm, division_string, divides, gcd, lcm, graded_order
...
class TestPolynomial(unittest.TestCase):
    ...
    def test__init__(self):
        ...
        self.assertTermMatrixEqual(t.term_matrix, [['constant', 'y', 'x'], [1.0, 1, 2], [1.0, 1, 0]])
    ...
    def test__iter__(self):
        S = Polynomial('x^2y + xy^2 + y^2')
        # All terms should have header and float coefficients
        expected = [
            Polynomial('x^2y'),
            Polynomial('xy^2'),
            Polynomial('y^2')
    ...
    def test_combine_variables(self):
        t = Polynomial('x')
        s = Polynomial('x^2y + xy^2 + y^2')
        res = (Polynomial('x^2y + xy^2 + y^2'), Polynomial('x'))
        self.assertEqual(Polynomial.combine_variables(s, t), res)
        self.assertEqual(s, Polynomial('x^2y + xy^2 + y^2'))
        self.assertEqual(t, Polynomial('x'))
    ...
    def test_division_algorithm(self):
        ...
        t = Polynomial('xy - 1')
        e = Polynomial('y^2 - 1')
        res1 = ([Polynomial('x + y'), Polynomial('1')], Polynomial('x + y + 1'))
    ...
    def test_division_string(self):
        ...
        res2 = 'x^2y + xy^2 + y^2 = (xy + y^2)*(x) + (y)*(y) + (remainder:) 0.0'
    ...
    def test_gcd(self):
        ...
        self.assertEqual(gcd(Polynomial('x^2-1'), Polynomial('x-1')), Polynomial('x-1'))
        self.assertEqual(gcd(Polynomial('x-1'), Polynomial('x^2-1')), Polynomial('x-1'))
        self.assertEqual(gcd(Polynomial('x', 2)**2 - Polynomial('x', 2), Polynomial('x^3 + x + 1', 2)), Polynomial('1'))
        self.assertEqual(gcd(Polynomial('x^4 + x^2 + 1.0', 2), Polynomial('x^4-x', 2)), Polynomial('x^2 + x + 1'))
        self.assertEqual(gcd(Polynomial('x', 2)**(2**3) - Polynomial('x', 2), Polynomial('x^5 + x^3 + 1.0', 2)), Polynomial('1'))
    ...
    def test__call__(self):
        f = Polynomial('xy+x^2+xz+y^2+yz+z^2+x+y+z+1')
        self.assertEqual(f(1, 2, 3), Polynomial('32'))
        g = Polynomial('2x^2+3y')
        self.assertEqual(g(2, 3), Polynomial('17'))
        self.assertEqual(g(2, 'y'), Polynomial('8 + 3y'))
        self.assertEqual(g(x=2), Polynomial('8 + 3y'))
        variables = {'x': 2, 'y': 1}
        self.assertEqual(g(**variables), Polynomial('11'))
        variables = {'x': 2}
    ...
    def test_gradient(self):
        ...
        # Always expect Polynomial objects with header for gradient evaluations
        self.assertEqual(f.grad(x=1, y=2), [Polynomial('12'), Polynomial('25')])
        f = Polynomial('x^2')
        self.assertEqual(f.grad(1), [Polynomial('2')])
    ...
    def test_hessian(self):
        ...
        self.assertEqual([[float(x) for x in row] for row in f.hessian(x=1, y=2)], [[4.0, 14.0], [14.0, 24.0]])
