...
class TestCLI(unittest.TestCase):
    ...
    def test_json_gcd(self):
        # gcd should work normally; JSON not defined for gcd, so just assert text path
        code, out, err = run_cli(["gcd", "x^2-1", "x-1", "-p", "0"])
        self.assertEqual(code, 0)
        self.assertIn("gcd(", out)

    def test_json_error_branches(self):
        # groebner invalid order
        code, out, err = run_cli(["--json", "groebner", "x^2+y^2-1", "x-y", "--order", "bogus"])
        self.assertEqual(code, 2)  # argparse usage error
        # solve-system with malformed polynomial
        code, out, err = run_cli(["--json", "solve-system", "x^2+-", "y-1"]) 
        self.assertEqual(code, 1)
        payload = json.loads(out)
        self.assertEqual(payload.get("status"), "error")
        self.assertIn("error", payload)
    ...
    def test_numeric_output_flags_groebner(self):
        ...
        code, out, err = run_cli(
            ["--rational", "groebner", "x^2+y^2-1", "x-y", "--order", "grevlex"]
        )
        self.assertEqual(code, 0)
        self.assertIn("Groebner basis:", out)
